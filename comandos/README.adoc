...
mas é preciso incluir a diretiva `:safe: unsafe` no topo do arquivo para habilitar isto.
...
A tag deve ter um nome, como desejarmos.
...
Depois de escolher o nome para sua tag, para abrir e fechá-la dentro do arquivo de código fonte,
basta incluir a linha `// tag::nome_da_tag[]` no início e no fim do bloco de código que deseja incluir.
